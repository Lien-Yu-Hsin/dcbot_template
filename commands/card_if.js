...
module.exports = {
    ...
    async execute(client, interaction) {
        ...
        if (area <0) {
            // 如何判斷 大或小
            await interaction.reply(`利奧拉輸 area=${area}`); // 如果符合，應該回覆甚麼訊息
        } else if (area>0) {
            // 應該要如何判斷剩下的
            await interaction.reply(`利奧拉贏 area=${area}`); // 如果符合，應該回覆甚麼訊息
        } else {
            // 最後一個應該是甚麼情況
            await interaction.reply(`平手 area=${area}`); // 如果符合，應該回覆甚麼訊息
        }
    },
};
